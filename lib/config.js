...
exports.getPrerenderServer = function getPrerenderServer( cfg ) {
    ...
    if (cluster.isMaster) {
        ...
        cluster.settings.cfg = cfg;             // Copy the cfg for the worker threads

    } else {

        cfg = cfg || cluster.settings.cfg;      // Yse the master copy of cfg instead of reloading every time
    }

    if ((server = prerender(cfg.options))) {     // Must be done per worker
...
exports.getConfig = function getConfig() {
    ...
    args
        ...
        .option('-NS, --removeScriptTags [boolean]' ,'Strip scripts out of rendered page, default true',function(value){
