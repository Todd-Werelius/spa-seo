...
exports.getConfig = function getConfig() {
    ...
    //1. Load the cmd line, and possibly the .json cfg file if present
    cmd = getCmdLineArgs(cfg,jsn);

    // 1. override defaults with any .json cfg properties
    cfg = cfgOverride(cfg,jsn);

    // 2. override defaults & .json cfg with any command line parameters
...
function getCmdLineArgs(cmd, jsn ) {
    ...
    args
        ...
        .option('-W , --workers [number]',"Number of CPU's to use : -1 to use all, default 1",function(value){
            ...
            cmd.options.workers = checkValueNumber(value,0,128,"workers must be 0 ( all CPU's ) or a positive number but was : "+value,this.help);
        ...
        .option('-I , --iterations [number]','Number of requests for restarting phantomJS service, default 200',function(value){
            cmd.options.iterations = checkValueNumber(value,1,500,args,"iterations must be a number between 1 and 500 but was : "+value,this.help);
            return cmd.options.iterations;
        })
        .option('-P , --port [number]','Port this server will listen on, default 3030',function(value){
            cmd.options.port = checkValueNumber(value,3000,64000,args,"port must be a number between 1000 and 6400 but was : "+value,this.help);
            return cmd.options.port;
        })
        .option('-B , --phantomBasePort [number]','Starting port for phantomjs cluster, default 12300',function(value){
            cmd.options.phantomBasePort = checkValueNumber(value,3001,64000,args,"phantomBasePort must be a number between 1000 and 6400 but was : "+value,this.help);
...
function checkValueNumber( value, upper, lower, onErrorMsg, helpFn ) {
