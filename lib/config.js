...
exports.getConfig = function getConfig() {
    ...
    args
        ...
        .option('-W , --workers [number]',"Number of CPU's to use : -1 to use all, default 1",function(value){
            ...
            cmd.options.workers = checkValueNumber(value,-1,64,true,"workers must be -1 or a number between 1 and maxCPU's but was : "+value,this.help);
        ...
        .option('-I , --iterations [number]','Number of requests for restarting phantomJS service, default 200',function(value){
            cmd.options.iterations = checkValueNumber(value,1,500,true,args,"iterations must be a number between 1 and 500 but was : "+value,this.help);
            return cmd.options.iterations;
        })
        .option('-P , --port [number]','Port this server will listen on, default 3030',function(value){
            cmd.options.port = checkValueNumber(value,3000,64000,true,args,"port must be a number between 1000 and 6400 but was : "+value,this.help);
            return cmd.options.port
        })
        .option('-B , --phantomBasePort [number]','Starting port for phantomjs cluster, default 12300',function(value){
            cmd.options.phantomBasePort = checkValueNumber(value,3001,64000,true,args,"phantomBasePort must be a number between 1000 and 6400 but was : "+value,this.help);
